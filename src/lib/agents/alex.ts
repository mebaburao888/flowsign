...
function runDemoFallback(messages: Message[], context: AlexContext): string {
  ...
  if (lastUserMessage.includes('16') || lastUserMessage.includes('m3 max') || lastUserMessage.includes('max')) {
    return `Got it — a few things to know about the MacBook Pro 16" M3 Max before you decide:\n\n• It's non-standard for your team (standard is ${device})\n• It's currently out of stock — procurement adds ${context.stockStatus.procurementDays ?? 7} days, which could delay your start\n• It requires approval from both your manager and IT\n\nIs this a hard requirement, or would the standard ${device} work for you?`
  }

  if (lastUserMessage.includes('hard requirement') || lastUserMessage.includes('need it') || lastUserMessage.includes('i insist')) {
    return `Got it — I've filed the exception request. Your manager and IT have both been notified and will review within 1 business day.\n\nIn the meantime, you can continue with the rest of your onboarding checklist — no need to wait on this. Check your inbox for status updates. [EXCEPTION_REQUESTED]`
  }
...
- Respond with ONE single message covering everything. Do NOT split this into multiple turns.
- In that one message include ALL of: (1) it's non-standard for their team, (2) it's out of stock and procurement adds ${stockStatus.procurementDays} days which risks their start date, (3) ask if it's a hard requirement or if the standard would work.
- Use bullet points inside that message to keep it scannable.
- If they say it's a hard requirement: confirm exception in ONE message and end with [EXCEPTION_REQUESTED]. Also tell them: "I've filed the exception. Your manager and IT will review within 1 business day. In the meantime, you can continue with the rest of your onboarding checklist — check your inbox for updates."
