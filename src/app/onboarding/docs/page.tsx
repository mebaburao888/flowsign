...
import { CheckCircle, Send, Loader2, FileText, ChevronRight, PenLine } from 'lucide-react'
...
export default function DocsPage() {
  ...
  // Reset Q&A history and scroll doc to top when switching docs
  useEffect(() => {
    setQaHistory([])
  ...
  async function loadData() {
    ...
    // Check already-signed docs
    const signedRes = await fetch(`/api/docs?employeeId=${EMPLOYEE_ID}`)
    // (signed state is returned from sign action — start empty and let sign update it)
  }
  ...
  async function handleSign() {
    ...
    try {
      ...
      if (data.allComplete) {
        ...
        // Advance to next doc after short delay
        setTimeout(() => {
          setCurrentIdx(i => i + 1)
        }, 600)
      }
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-10 max-w-md w-full text-center">
          ...
          <p className="text-slate-500 text-sm mb-2">Your manager has been notified. Let's get your laptop sorted next.</p>
          <div className="space-y-2 text-left my-6">
            {docs.map(doc => (
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* Main layout: doc panel + Q&A panel */}
          ...
          <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3">
            {qaHistory.length === 0 && (
              <div className="text-center py-6">
                ...
                <p className="text-slate-400 text-xs leading-relaxed">Questions about the <strong>{currentDoc.title}</strong>? Ask me anything.</p>
            ...
            {qaHistory.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'assistant' && (
                  <div className="w-6 h-6 bg-gradient-to-br from-brand-400 to-brand-600 rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0 mr-2 mt-1">
                    A
                  </div>
                )}
            ...
            {qaLoading && (
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 bg-gradient-to-br from-brand-400 to-brand-600 rounded-full flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
                  A
                </div>
